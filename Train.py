...
from Utils import save_checkpoint, load_checkpoint, mkdir
from Dataset import ABDataset
from Model import Generator, Discriminator

import gc
from multiscale import Downscale2, Downscale4
...
def train_fn(disc_A, disc_B, gen_A, gen_B, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler, epoch,
             downscale2, downscale4):
    global count
    avg_dloss = 0
    avg_gloss = 0
    loop = tqdm(loader, leave=True)
    if is_MULTI_SCALE:
        print("=> loading multiscale module...")
    for idx, (a, b) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            b_multiscale_loss = 0
            if is_MULTI_SCALE:
                cycle_a_down2 = downscale2(cycle_a)
                cycle_a_down4 = downscale4(cycle_a)
                real_a_down2 = downscale2(a)
                real_a_down4 = downscale4(a)
                a_multiscale_loss = l1(real_a_down2, cycle_a_down2) + \
                                    l1(real_a_down4, cycle_a_down4)

                cycle_b_down2 = downscale2(cycle_b)
                cycle_b_down4 = downscale4(cycle_b)
                real_b_down2 = downscale2(b)
                real_b_down4 = downscale4(b)
                b_multiscale_loss = l1(real_b_down2, cycle_b_down2) + \
                                         l1(real_b_down4, cycle_b_down4)
            ...
            G_loss = (
                ...
                + a_multiscale_loss * LAMBDA_MULTI_SCALE
                + b_multiscale_loss * LAMBDA_MULTI_SCALE
            )
        ...
        mkdir(path)
        mkdir(path+"/Generated from rain")
        mkdir(path+"/Generated from norain")

        if idx % 200 == 0:
            save_image(fake_a*0.5+0.5, f"{path}/Generated from rain/fake_{idx}.png")
            save_image(fake_b*0.5+0.5, f"{path}/Generated from norain/fake_{idx}.png")
            save_image(b*0.5+0.5, f"{path}/Generated from rain/real_{idx}.png")
            save_image(a*0.5+0.5, f"{path}/Generated from norain/real_{idx}.png")
...
def main():
    ...
    # multiscale model
    downscale2 = Downscale2(in_channels=3).to(DEVICE)
    downscale4 = Downscale4(in_channels=3).to(DEVICE)
    ...
    dataset = ABDataset(
        root_a=TRAIN_DIR+"/norain", root_b=TRAIN_DIR+"/rain", transform=transforms
    )
...
if __name__ == "__main__":
    ...
    TRAIN_DIR = "datasets/Rain200L/train"
    path = "Results"
    BATCH_SIZE = 1
    LEARNING_RATE = 1e-5
    LAMBDA_IDENTITY = 10
    ...
    # 新增模块multiscale
    is_MULTI_SCALE = True
    LAMBDA_MULTI_SCALE = 10
    # 新增模块se-attention
    is_SE_ATTENTION = True
